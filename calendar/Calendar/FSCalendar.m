...
const char * flowKey;
const NSInteger  weekdayLabelTag = 13000;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _rowNum = 6;
        _beginDate = [NSDate dateWithTimeIntervalSinceNow:- 24 * 60 * 60 * 3];
        [self initialize];
    }
    return self;
}
...
- (void)initialize
{
    ...
    _page0 = [[FSCalendarPage alloc] initWithFrame:CGRectZero rowNum:_rowNum beginDate:_beginDate] ;
    _page0.tag = 0;
    [_scrollView addSubview:_page0];
    
    _page1 = [[FSCalendarPage alloc] initWithFrame:CGRectZero  rowNum:_rowNum beginDate:_beginDate];
    _page1.tag = 1;
...
-(void)setWeekSymbols:(NSArray *)weekSymbols{
    
    if ([_weekSymbols isEqualToArray:weekSymbols]) {
        return;
    }
//    移除原有视图
    for (int i = 0 ; i < _weekSymbols.count; i ++ ) {
        UILabel *view = (UILabel *)[self viewWithTag:weekdayLabelTag + i ] ;
        [_weekdays removeObject:view];
...
//    新增新的视图
    ...
    for (int i = 0; i < weekSymbols.count; i++) {
        ...
        weekdayLabel.font = _weekdayFont;
        weekdayLabel.textColor = kBlueText;
        weekdayLabel.tag = weekdayLabelTag + i;
        [_weekdays addObject:weekdayLabel];
        [self addSubview:weekdayLabel];
    }
}
