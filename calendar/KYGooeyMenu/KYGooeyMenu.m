...
-(void)setUpSomeDatas{
    ...
    for (NSInteger i = 0; i < menuCount; i++) {
        CGFloat cosDegree = - sinf(degree * (i+1)) ;
        CGFloat sinDegree = - cosf(degree * (i+1)) ;
        ...
        if (_imgNameArr.count == menuCount  ) {
            UIImageView *imgView  = [[UIImageView alloc]initWithFrame:item.bounds];
            imgView.image = [UIImage imageNamed:_imgNameArr[i]];
...
- (CGPathRef) getRightLinePathWithAmount:(CGFloat)amount {
    ...
    CGPoint pointO = CGPointMake(self.mainView.frame.origin.x - amount * cosf(45 *(M_PI/180)), self.mainView.frame.origin.y + self.mainView.bounds.size.height + amount * cosf(45 *(M_PI/180)));
    
    [verticalLine moveToPoint:pointB];
    [verticalLine addQuadCurveToPoint:pointA controlPoint:pointO];
    [verticalLine addArcWithCenter:self.mainView.center radius:R startAngle:M_PI_2*3 endAngle:M_PI clockwise:NO];
    [verticalLine addArcWithCenter:self.mainView.center radius:R startAngle:M_PI endAngle:0 clockwise:NO];
...
-(void)menuTap:(UITapGestureRecognizer *)tapGes{
    ...
    for (NSInteger i = 0; i<menuCount; i++) {
        if ((tapGes.view.tag == i+1) && [self.menuDelegate respondsToSelector:@selector(menuDidSelected:)]) {
            [self.menuDelegate menuDidSelected:menuCount - i - 1];
        }
    }
...
//        morph_left.beginTime = CACurrentMediaTime()+0.3f;
        morph_left.values = values1_0_left;
        morph_left.duration = 0.4f;
        morph_left.removedOnCompletion = NO;
        morph_left.fillMode = kCAFillModeForwards;
        morph_left.delegate = self;
...
//        morph_right.beginTime = CACurrentMediaTime()+0.3f;
        morph_right.values = values0_1_left;
        morph_right.duration = 0.1f;
        morph_right.removedOnCompletion = NO;
        morph_right.fillMode = kCAFillModeForwards;
        morph_right.delegate = self;
